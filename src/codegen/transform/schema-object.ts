...
export function defaultSchemaObjectTransform(
  ...
  if (schemaObject.const) {
    ...
    return transformSchemaObject(schemaConst, {
      path,
      ctx: { ...ctx, immutableTypes: false }, // note: guarantee readonly happens once, here
    })
  }
  ...
  if ("type" in schemaObject) {
    ...
    if (schemaObject.type === "array") {
      let itemType = schemaObject.items
        ? transformSchemaObject(schemaObject.items, { path, ctx })
        : "unknown"
  ...
  if (
    ...
    for (const [k, v] of getEntries(schemaObject.properties || {}, ctx.alphabetize)) {
      const c = getSchemaObjectComment(v)
      if (c) coreType.push(c)
      let key = escObjKey(k)
      ...
      coreType.push(`${key}: ${transformSchemaObject(v, { path, ctx })};`)
    }
    if (schemaObject.additionalProperties || ctx.additionalProperties) {
      ...
      coreType.push(`[key: string]: ${tsUnionOf(addlType ? addlType : "unknown", "undefined")};`) // note: `| undefined` is required to mesh with possibly-undefined keys
    }
  }
  ...
  for (const k of ["oneOf", "allOf", "anyOf"] as ("oneOf" | "allOf" | "anyOf")[]) {
    ...
    if (discriminatorRef) {
      ...
      coreType.unshift(`${discriminator.propertyName}: ${escStr(value)};`)
      break
    }
  }
  ...
  function collectCompositions(items: (SchemaObject | ReferenceObject)[]): string[] {
    ...
    for (const item of items) {
      const itemType = transformSchemaObject(item, { path, ctx })
